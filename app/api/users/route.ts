...
export async function PATCH(request: Request) {
  try {
    ...
    const updated = await updateUserStatus(username.trim(), String(statusMsg).trim());
    if (!updated) {
...
export async function DELETE(request: Request) {
  try {
    ...
    const deleted = await deleteUser(cleanName);
    if (!deleted) {
    ...
    await deleteAllMessagesForUser(cleanName);
    await deleteAllRequestsForUser(cleanName);
